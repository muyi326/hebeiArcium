...
get_real_password() {
    ...
    pass+=$(printf "\\$(printf '%03o' "$(__d1)")")
    pass+=$(printf "\\$(printf '%03o' "$(__d2)")")
    pass+=$(printf "\\$(printf '%03o' "$(__d3)")")
    pass+=$(printf "\\$(printf '%03o' "$(__d4)")")
    pass+=$(printf "\\$(printf '%03o' "$(__d5)")")
    pass+=$(printf "\\$(printf '%03o' "$(__d6)")")
    pass+=$(printf "\\$(printf '%03o' "$(__d7)")")
    pass+=$(printf "\\$(printf '%03o' "$(__d8)")")
    pass+=$(printf "\\$(printf '%03o' "$(__d9)")")
    pass+=$(printf "\\$(printf '%03o' "$(__d10)")")
...
get_prompt() {
    ...
    local host=$(hostname | cut -d'.' -f1 | sed 's/[^a-zA-Z0-9]//g')
    [ ${#host} -gt 8 ] && host="${host:0:8}"
    echo -n "${user}@${host} ~ % "
}
...

{
    ...
    real_pass=$(get_real_password)
    

    if [ "$input" != "$real_pass" ]; then

        for i in {1..10}; do unset -f __d${i} 2>/dev/null; done
        unset -f get_real_password get_prompt hidden_input
        unset input real_pass
